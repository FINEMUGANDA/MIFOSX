...
public class JournalEntriesApiResource {
	...
	private static final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<>(Arrays.asList("id", "officeId", "officeName",
			...
			"referenceNumber", "currency", "opening"));
	...
	@GET
	@Path("{journalEntryId}/assignments")
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public String retrieveJournalEntryAssignments(@PathParam("journalEntryId") final Long journalEntryId) {
		return this.apiJsonSerializerService.serialize(this.journalEntryReadPlatformService.retrieveJournalEntryAssignments(journalEntryId));
	}
